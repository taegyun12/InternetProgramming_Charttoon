...
String author=request.getParameter("author"); //작가명으로 검색할 시 전 페이지에서 author을 매개변수로 받음 - 테이블에서 검색하기 위함
...
try {
    ...
    String query = "select idx,title,genre,author,savedFileName from webtoonList where author like '%" + author + "%'"; //author을 작가명에 포함하는 모든 웹툰 정보를 검색
    rs = stmt.executeQuery(query); //해당하는 웹툰들의 정보를 가져옴

%>
<section>
<div class="enrollTitle">웹툰 리스트</div>
<div class="rankTable"></div>
<table border="1" class="detailTable"> <!-- 테이블 형식으로 출력 -->
